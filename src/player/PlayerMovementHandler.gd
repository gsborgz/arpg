...
func _handle_sprint() -> void:
	if Input.is_action_pressed("sprint") and character.is_on_floor():
		speed = SPRINT_SPEED * CROUCH_SPEED_MODIFIER if is_crouching else SPRINT_SPEED 
		is_sprinting = true
	else:
		speed = WALK_SPEED * CROUCH_SPEED_MODIFIER if is_crouching else WALK_SPEED
		is_sprinting = false
